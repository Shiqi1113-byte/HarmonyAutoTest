...
// 引入 Tree-sitter C API
#include <tree_sitter/api.h>

class ModulesManager {
public: // 将内部结构体设为 public 方便外部测试读取
    /*
        函数依赖于文件
        文件之间存在依赖关系
    ...
    */
    ...
    struct FileNode {
        ...
        // Tree-sitter 解析入口（将在 .cpp 中实现）
        void parse();

        // Tree-sitter 解析辅助函数
        void traverseAST(TSNode node, const std::string& source_code);
        std::string getNodeText(TSNode node, const std::string& source_code);
...
public:
    ...
    void scanModule(const std::string &root_path) {
        ...
        for (auto it = fs::recursive_directory_iterator(root_path);
            ...
            if (path.extension() == ".ets" || path.extension() == ".ts") {
                FileNode *cur_file = getFile(path.string());
                cur_file->module_path = root_path;
                cur_file->parse(); // 触发 Tree-sitter 解析
            }
        }
    }
...
public:
    std::map<std::string, FileNode*> files_pool; // 路径 -> 文件 (改为 public 方便打印测试)
